...
<?php
if(!empty($_POST['login']) || !empty($_POST['address']) || !empty($_POST['city']) || !empty($_POST['zcode']) || !empty($_POST['password']) || !empty($_POST['password_check']) || !empty($_POST['email'])) {
    ...
    if(empty($_POST['zcode'])) {
        $_POST['zcode'] = $_SESSION['zcode'];
    }
    if(empty($_POST['password'])) {
        $_POST['password'] = $_SESSION['password'];
        $_POST['password_check'] = $_SESSION['password_check'];
    }
    if(empty($_POST['email'])) {
        $_POST['email'] = $_SESSION['email'];
    }
    if((strlen($_POST['login']) >= 2) && (strlen($_POST['city']) >= 3) && (strlen($_POST['zcode']) == 5) && is_numeric($_POST['zcode']) && (strlen($_POST['password']) >= 6) && ($_POST['password_check'] === $_POST['password'])) {
        ...
        if ($count['nb_user']) {
            ...
            $user->updateUser($_SESSION['user']['id'], $_POST['login'], $_POST['password'], $_POST['address'], $_POST['city'], $_POST['zcode'], $_POST['email'], $_POST['newsletter'], $_POST['tel']);
